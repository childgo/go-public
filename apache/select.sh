...
alsco_get_domain=$(cut -d "@" -f2 <<< "$email_id36") ; echo "$result_user"
...
uapi --output=jsonpretty --user=$alsco_get_user Email passwd_pop email=$email_id36 password=$final_email_password
