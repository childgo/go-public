...
options=("Grep WHM root IP login for 2022 1" "search for python and ini 2" "Grep words and exclude some files extension 3" "find files older than 20 days 4" "grep SecRuleEngine Off from userdata 5" "find every symbolic link on a server 6" "List all connected SSH sessions 7" "find all hidden files 8" "check all immutable files 9" "search for htaccess 10" "search for asp 11" "search for sh 12" "search for txt 13" "search for log 14" "grep TCP_IN 15" "grep Hosts.allow 16" "Option 16" "Quit")
select opt in "${options[@]}"
do
case $opt in

########################################################
"Grep WHM root IP login for 2022 1")
echo "grep  2022 //usr/local/cpanel/logs/access_log | grep -E '\sroot\s' | cut -f1 -d\  | sort -u"
echo " "
echo "Logins for the cPanel software are logged at the following file"
echo "grep "68.183.188.48" /usr/local/cpanel/logs/session_log"
echo " "



grep  2022 //usr/local/cpanel/logs/access_log | grep -E '\sroot\s' | cut -f1 -d\  | sort -u
;;
########################################################
