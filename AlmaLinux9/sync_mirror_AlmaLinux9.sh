...
LOCAL_REPOS="baseos appstream extras epel crb"
...
for REPO in ${LOCAL_REPOS}; do
    ...
    sudo dnf reposync --repo=$REPO --newest-only --download-metadata --download-path=$ALSCO_Path/$REPO --delete
    ...
    if [[ $REPO = 'baseos' || $REPO = 'appstream' ]]; then
        COMPS_FILE="$ALSCO_Path/$REPO/comps.xml"
        if [ -f "$COMPS_FILE" ]; then
            sudo createrepo_c -g "$COMPS_FILE" $ALSCO_Path/$REPO/
        else
