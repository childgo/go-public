...
"Check Internet Speed 36")
echo "Ckecking Internet Speed..."

curl -4 -L -o /dev/null -w $'\nDownloaded: %{size_download} bytes\nSpeed: %{speed_download} bytes/sec\nTime: %{time_total} sec\n' https://ash-speed.hetzner.com/100MB.bin
...
# ----------------------------------------------------------
get_max_fails() {
    grep -oP '^\s*local\s+max_fails\s*=\s*\K[0-9]+' "$CHECK_LUA" 2>/dev/null | tail -n1
}

get_ban_window() {
    grep -oP '^\s*local\s+ban_window_seconds\s*=\s*\K[0-9]+' "$LOG_LUA" 2>/dev/null | tail -n1
}
...
# ----------------------------------------------------------
echo "----------------------------------------------"
sed -n 's/.*IP: \([0-9]*\.[0-9]*\.[0-9]*\)\.[0-9]*,.*/\1.0\/24/p' /tmp/SG_Login_Block.log | sort | uniq -c | sort -rn | head
echo "----------------------------------------------"

realtime_monitor() { watch -n 5 'echo "=== TOP FAILED IPs ==="; grep "STATUS=FAILED" /tmp/SG_Login_Attempts.log | grep -oP "ip=\K[^ ]+" | sort | uniq -c | sort -rn | head -20; echo; echo "=== TOP TARGETED USERNAMES ==="; grep "STATUS=FAILED" /tmp/SG_Login_Attempts.log | grep -oP "user=\K[^ ]+" | sort | uniq -c | sort -rn | head -20; echo; echo "=== BLOCKED IPs (all-time) ==="; grep -oP "IP: \K[^,]+" /tmp/SG_Login_Block.log | sort | uniq -c | sort -rn; echo; echo "=== CURRENTLY BLOCKED ==="; gawk -v ban=600 -v maxf=3 '\''{ if (match($0, /^\[([0-9-]+) ([0-9:]+)\]/, tsm)) { split(tsm[1], d, "-"); split(tsm[2], t, ":"); ts = mktime(d[1]" "d[2]" "d[3]" "t[1]" "t[2]" "t[3]) } else next; match($0, /STATUS=([A-Z]+)/, sm); status = sm[1]; match($0, /ip=([^ ]+)/, im); ip = im[1]; if (status == "SUCCESS") { count[ip] = 0 } else if (status == "FAILED") { if (ts - last[ip] > ban) count[ip] = 1; else count[ip]++; last[ip] = ts } } END { now = systime(); found = 0; for (ip in count) { if (count[ip] >= maxf) { remaining = ban - (now - last[ip]); if (remaining > 0) { printf "%-18s fails=%-3d unblocks in %dm%02ds\n", ip, count[ip], int(remaining/60), remaining%60; found = 1 } } } if (!found) print "(none currently blocked)" }'\'' /tmp/SG_Login_Attempts.log'; }
